...
import me.chanjar.weixin.mp.api.impl.lock.AquiredLockWorker;
import me.chanjar.weixin.mp.api.impl.lock.DistributedLocker;
  ...
  public String getTicket(TicketType type, boolean forceRefresh) throws WxErrorException {
    ...
    distributedLocker.lock("test",new AquiredLockWorker<Object>() {
      ...
      public Object invokeAfterLockAquire() {
        try {
          if (forceRefresh) {
            wxMpConfigStorage.expireTicket(type);
          }

          if (wxMpConfigStorage.isTicketExpired(type)) {
            String responseContent = execute(SimpleGetRequestExecutor.create(this),
                    WxMpService.GET_TICKET_URL + type.getCode(), null);
            JsonObject tmpJsonObject = JSON_PARSER.parse(responseContent).getAsJsonObject();
            String jsapiTicket = tmpJsonObject.get("ticket").getAsString();
            int expiresInSeconds = tmpJsonObject.get("expires_in").getAsInt();
            wxMpConfigStorage.updateTicket(type, jsapiTicket, expiresInSeconds);
          }
        }catch (Exception e){
          e.printStackTrace();
        }
